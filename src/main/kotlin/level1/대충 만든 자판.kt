...
private fun solution(keymap: Array<String>, targets: Array<String>): IntArray {
    ...
    for (target in targets) {
        var an = 0
        for (t in target) {
            val keyIndex = mutableListOf<Int>()
            for (key in keymap) {
                if (key.indexOf(t) != -1) {
                    keyIndex += key.indexOf(t) + 1
                }
            }
            if (keyIndex.isEmpty()) {
                an = -1
...
fun main() {
    ...
    val answer = intArrayOf(9, 4)

    checkAnswer(solution(keymap, targets).contentEquals(answer))
}