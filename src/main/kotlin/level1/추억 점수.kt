...
각 사진에 찍힌 인물의 이름을 담은 이차원 문자열 배열 photo가 매개변수로 주어질 때, 사진들의 추억 점수를 photo에 주어진 순서대로
...
private fun solution(name: Array<String>, yearning: IntArray, photo: Array<Array<String>>): IntArray {
    ...
    for (ph in photo) {
        answer += ph.sumOf { if (map[it] != null) map[it]!! else 0 }
    }
    return answer
}
...
fun main() {
    ...
    val photo = arrayOf(
        arrayOf("may", "kein", "kain", "radi"), arrayOf("may", "kein", "brin", "deny"),
        arrayOf("kon", "kain", "may", "coni")
    )
    val answer = intArrayOf(19, 15, 6)

    checkAnswer(solution(name, yearning, photo).contentEquals(answer))
}